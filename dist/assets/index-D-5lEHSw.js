(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();function e(e,t=!1){return window.__TAURI_INTERNALS__.transformCallback(e,t)}async function t(e,t={},n){return window.__TAURI_INTERNALS__.invoke(e,t,n)}var n=class extends HTMLElement{constructor(...e){super(...e),this.currentView=`list`,this.ollamaConnected=!1}connectedCallback(){this.render(),this.addEventListener(`navigate`,(e=>{this.navigateTo(e.detail)})),this.checkOllamaStatus(),setInterval(()=>this.checkOllamaStatus(),3e4)}async checkOllamaStatus(){try{this.ollamaConnected=await t(`check_ollama_status`)}catch{this.ollamaConnected=!1}this.updateStatusIndicator()}updateStatusIndicator(){let e=this.querySelector(`#ollama-status`);e&&(e.className=`status-indicator ${this.ollamaConnected?`status-connected`:`status-disconnected`}`,e.textContent=this.ollamaConnected?`● Ollama connecté`:`○ Ollama indisponible`)}navigateTo(e){this.currentView=e.view;let t=this.querySelector(`#app-content`);if(t)switch(e.view){case`list`:t.innerHTML=`<journal-list></journal-list>`;break;case`editor`:e.id?t.innerHTML=`<journal-editor entry-id="${e.id}"></journal-editor>`:t.innerHTML=`<journal-editor></journal-editor>`;break;case`entry`:t.innerHTML=`<journal-entry-view entry-id="${e.id}"></journal-entry-view>`;break;case`search`:t.innerHTML=`<journal-search></journal-search>`;break;case`chat`:e.sessionId?t.innerHTML=`<chat-view session-id="${e.sessionId}"></chat-view>`:e.entryId?t.innerHTML=`<chat-view entry-id="${e.entryId}"></chat-view>`:t.innerHTML=`<chat-view></chat-view>`;break;case`chat-history`:t.innerHTML=`<chat-session-list></chat-session-list>`;break}}render(){this.innerHTML=`
    ...
    `,this.querySelector(`#nav-journal`)?.addEventListener(`click`,()=>{this.navigateTo({view:`list`})}),this.querySelector(`#nav-new-entry`)?.addEventListener(`click`,()=>{this.navigateTo({view:`editor`})}),this.querySelector(`#nav-search`)?.addEventListener(`click`,()=>{this.navigateTo({view:`search`})}),this.querySelector(`#nav-chat`)?.addEventListener(`click`,()=>{this.navigateTo({view:`chat`})}),this.querySelector(`#nav-chat-history`)?.addEventListener(`click`,()=>{this.navigateTo({view:`chat-history`})})}};customElements.define(`app-shell`,n);async function r(e){return t(`create_entry`,{input:{body:e}})}async function i(e){return t(`get_entry`,{id:e})}async function a(e=0,n=50){return t(`list_entries`,{input:{offset:e,limit:n}})}async function o(e,n){return t(`update_entry`,{input:{id:e,body:n}})}async function s(e){return t(`delete_entry`,{id:e})}async function c(e){return t(`search_entries`,{query:e})}var l=class extends HTMLElement{constructor(...e){super(...e),this.entries=[]}connectedCallback(){this.render(),this.loadEntries()}async loadEntries(){this.entries=await a(),this.render()}render(){if(this.entries.length===0){this.innerHTML=`
    ...
    `,this.querySelector(`#chat-back-btn`)?.addEventListener(`click`,()=>{this.dispatchEvent(new CustomEvent(`navigate`,{detail:{view:`list`},bubbles:!0}))}),this.querySelector(`#send-btn`)?.addEventListener(`click`,()=>{this.handleSend()}),this.querySelector(`#chat-input`)?.addEventListener(`keydown`,e=>{let t=e;t.key===`Enter`&&!t.shiftKey&&(t.preventDefault(),this.handleSend())})}renderMessages(){let e=this.querySelector(`#chat-thread`);e&&(e.innerHTML=this.messages.map(e=>{let t=e.role===`user`,n=t?`Vous`:`Psychly`;return`
    ...
    `,this.querySelector(`#disclaimer-accept`)?.addEventListener(`click`,()=>{localStorage.setItem(S,`true`),this.remove()})}};customElements.define(`disclaimer-dialog`,C);